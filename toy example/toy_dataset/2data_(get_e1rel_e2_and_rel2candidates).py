...
def candidate_triples(dataset):
    ...
    test2_tasks = json.load(open(dataset + '/test2_tasks.json'))

    all_reason_relations = list(train_tasks.keys()) + list(dev_tasks.keys()) + list(test_tasks.keys()) + list(test2_tasks.keys())

    all_reason_relation_triples = list(train_tasks.values()) + list(dev_tasks.values()) + list(test_tasks.values()) + list(test2_tasks.values())
...
def for_filtering(dataset, save=False):
    ...
    test2_tasks = json.load(open(dataset + '/test2_tasks.json'))
    few_triples = []
    for _ in (list(train_tasks.values()) + list(dev_tasks.values()) + list(test_tasks.values()) + list(test2_tasks.values())):
        few_triples += _
