...
df_drugs_smiles = pd.read_csv('dataset2/drugSMLIES.csv')
DRUG_TO_INDX_DICT = {drug_id: indx for indx, drug_id in enumerate(df_drugs_smiles['id'])}
DRUG_INDX_NAME_DICT = {indx: drug_id  for indx, drug_id in enumerate(df_drugs_smiles['id'])}
drug_id_mol_graph_tup = [(id, Chem.MolFromSmiles(smiles.strip())) for id, smiles in zip(df_drugs_smiles['id'], df_drugs_smiles['iosSMILES'])]
drug_to_mol_graph = {id:Chem.MolFromSmiles(smiles.strip()) for id, smiles in zip(df_drugs_smiles['id'], df_drugs_smiles['iosSMILES'])}
...
fp = np.load('fp/features/morgan_dataset2.npz')
fp = fp['features']
