...
try:
    df_drugs_smiles = pd.read_csv('twoside/PubChem_SMLIES.csv')
except:
    df_drugs_smiles = pd.read_csv('twoside/drugDB_list.csv')
DRUG_TO_INDX_DICT = {drug_id: indx for indx, drug_id in enumerate(df_drugs_smiles['CID'])}
DRUG_INDX_NAME_DICT = {indx: drug_id  for indx, drug_id in enumerate(df_drugs_smiles['CID'])}
drug_id_mol_graph_tup = [(id, Chem.MolFromSmiles(smiles.strip())) for id, smiles in zip(df_drugs_smiles['CID'], df_drugs_smiles['CanonicalSMILES'])]
drug_to_mol_graph = {id:Chem.MolFromSmiles(smiles.strip()) for id, smiles in zip(df_drugs_smiles['CID'], df_drugs_smiles['CanonicalSMILES'])}
...
fp = np.load('fp/features/morgan_twoside.npz')
fp = fp['features']
